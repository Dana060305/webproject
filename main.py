...
weather_img = {'Clear': ['static/img/state_weather/clearsky.png', 'Ясно'],
               'Few_clouds': ['static/img/state_weather/fewclouds.png',
                              'Переменная облачность'],
               'Scattered_clouds': [
                   'static/img/state_weather/scatteredclouds.png',
                   'Облачно с прояснениями'],
               'Broken_clouds': ['static/img/state_weather/brokenclouds.png',
                                 'Пасмурно'],
               'Drizzle': ['static/img/state_weather/showerrain.png',
                           'Моросящий дождь'],
               'Rain': ['static/img/state_weather/rain.png', 'Дождь'],
               'Thunderstorm': ['static/img/state_weather/thunderstorm.png',
                                'Гроза'],
               ...
               'Ash': ['static/img/state_weather/mist.png',
                       'Вулканическая пыль'],
...
def profile():
    ...
    user = sessions.query(users.User).filter(
        users.User.id == current_user.get_id()).first()
...
def profile_update():
    ...
    if request.method == 'POST':
        ...
        return render_template('profile_update.html',
                               title='Обновление профиля', form=form,
                               message='')
    return render_template('profile_update.html', title='Авторизация',
                           form=form, message='')
...
def register():
    ...
    if request.method == 'POST':
        ...
        f.save(os.path.join(
            'static/img_users/' + ''.join(form.email.data.split('.')) + '.png'))
        user = users.User(
            name=form.name.data,
            email=form.email.data,
            password=form.password.data,
            img='static/img_users/' + ''.join(
                form.email.data.split('.')) + '.png',
...
@app.route('/')
def delete():
    return render_template("main_display.html", title='Главная',
                           message='Главная')
...
def abxazia():
    ...
    data = ''
    sessions = db_session.create_session()
    data = (sessions.query(Reviews.Review.name, Reviews.Review.text,
                           Reviews.Review.img).filter(
        Reviews.Review.place == 'АБХАЗИЯ'))[::]
    if request.method == 'POST':
        name = sessions.query(users.User.name, users.User.img).filter(
            users.User.id == current_user.get_id()).first()
        rev = Reviews.Review(
            name=name[0],
            text=form.text.data,
    ...
    information = \
    sessions.query(Place.info).filter(Place.name == 'АБХАЗИЯ').first()[0]
    ...
    return render_template('abxazia.html', title='Абхазия',
                           information=information, weather=weather, form=form,
                           data=data)
...
def kabardino_balkaria():
    ...
    information = sessions.query(Place.info).filter(
        Place.name == 'КАБАРДИНО-БАЛКАРИЯ').first()[0]
    weather = Searching(43.485259, 43.607072).return_weather().split()
    img, description = weather_img[weather[1]]
    weather = [str(weather[0]) + "°"] + [description] + [img]
    return render_template('kabardino_balkaria.html',
                           title='Кабардино-Балкария', information=information,
                           form=form,
...
def krasnodarskiy_kray():
    ...
    data = (sessions.query(Reviews.Review.name, Reviews.Review.text,
                           Reviews.Review.img).filter(
        Reviews.Review.place == 'КРАСНОДАРСКИЙ КРАЙ'))[::]
    if request.method == 'POST':
        name = sessions.query(users.User.name, users.User.img).filter(
            users.User.id == current_user.get_id()).first()
        rev = Reviews.Review(
    ...
    information = sessions.query(Place.info).filter(
        Place.name == 'КРАСНОДАРСКИЙ КРАЙ').first()[0]
    weather = Searching(45.035470, 38.975313).return_weather().split()
    img, description = weather_img[weather[1]]
    weather = [str(weather[0]) + "°"] + [description] + [img]
    return render_template('krasnodarskiy_kray.html',
                           title='Краснодарский край', information=information,
                           form=form,
                           data=data, weather=weather)
...
def crimea():
    ...
    information = \
    sessions.query(Place.info).filter(Place.name == 'КРЫМ').first()[0]
    weather = Searching(44.948237, 34.100318).return_weather().split()
    img, description = weather_img[weather[1]]
    weather = [str(weather[0]) + "°"] + [description] + [img]
    return render_template('crimea.html', title='Крым', information=information,
...
def karelia():
    ...
    information = \
    sessions.query(Place.info).filter(Place.name == 'КАРЕЛИЯ').first()[0]
    weather = Searching(61.787374, 34.354325).return_weather().split()
    img, description = weather_img[weather[1]]
    weather = [str(weather[0]) + "°"] + [description] + [img]
    return render_template('karelia.html', title='Карелия',
                           information=information, weather=weather, form=form,
                           data=data)
...
def adygeya():
    ...
    information = sessions.query(Place.info).filter(
        Place.name == 'РЕСПУБЛИКА АДЫГЕЯ').first()[0]
    weather = Searching(44.608865, 40.098548).return_weather().split()
    img, description = weather_img[weather[1]]
    weather = [str(weather[0]) + "°"] + [description] + [img]
    return render_template('adygeya.html', title='Адыгея',
                           information=information, weather=weather, form=form,
                           data=data)
